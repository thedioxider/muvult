...
def test_identical_quality_is_not_better():
    # Equal quality must not count as better -- an equal-quality re-upload keeps
    # the existing pooled copy instead of overwriting it.
    assert is_better(320, "mp3", 320, "mp3") is False
    assert is_better(900, "flac", 900, "flac") is False
